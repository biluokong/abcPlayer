...
public class FqToAbcConverter {
     ...
     * @return ABC谱格式字符串
     */
    public static String convert(String fqStr) {
        // 重置状态变量
        ...
        String head = convertHead(headList, context);
        String body = convertBody(bodyList, context);
     ...
     * 转换头部信息
     */
    private static String convertHead(List<String> headList, ConverterContext context) {
        String one = "C";
        ...
        for (String line : headList) {
            if (line.startsWith("B:")) {        // 标题
                ...
                if (split[1].equals("8") && split[0].equals("6")) {
                    context.mergeUnit = Fraction.getFraction(3, 8);
                } else if (split[1].equals("8")) {
                    context.mergeUnit = Fraction.getFraction(1, 8);
                }
     ...
     * 构建音符映射表
     */
    private static Map<Character, String> buildLetterMap(String one) {
        Map<Character, String> letterMap = new HashMap<>();
        letterMap.put('0', "z");

        String[] letterArr = LETTER_ARR_UP;
        if (one.endsWith("$")) {
            letterArr = LETTER_AR_DOWN;
        }
        ...
        for (int i = 1; i <= 7; i++) {
            ...
            letterMap.put((char) (i + '0'), letterArr[idx] + suffix);
            if (i == 3 || i == 7) {
                idx++;
            } else {
                idx += 2;
     ...
     * 转换主体内容
     */
    private static String convertBody(List<String> bodyList, ConverterContext context) {
        ...
        for (String line : bodyList) {
            ...
            // 转换当前行
            String convertedLine = convertLine(currLine, context);
     ...
     * 转换单行
     */
    private static String convertLine(String line, ConverterContext context) {
        ...
        while (i < n) {
            ...
            if (c >= '0' && c <= '7') {
                ...
                if (j > i + 1) {
                    modifierStr.append(line, i + 1, j);
                }
                handleModifier(context.letterMap.get(c), modifierStr.toString(), sb, context);
                i = j;
            } else {
     ...
     * 处理小节线等段落结束标记
     */
    private static int handleSectionEnd(String line, char c, int i, StringBuilder sb, ConverterContext context) {
        ...
        if (c == ':' || c == '|') {  // 小节线：":|" "|" "||" "|:" "||/"
            // 分节后重新计算时值
            sb.append(context.tempSection.trim());
            context.tempSection = "";
            context.sumTime = Fraction.getFraction(0);

            int j = i + 1;
            while (j < line.length() && !context.letterMap.containsKey(line.charAt(j))) {
                j++;
            }
     ...
     * 处理倚音
     */
    private static String handleAppoggiatura(String str, Map<Character, String> letterMap) {
        StringBuilder sb = new StringBuilder();
        int i = 0;
        int n = str.length();
        ...
        while (i < n) {
            ...
            if (c >= '0' && c <= '7') {
                ...
                int j = i + 1;
                StringBuilder modifierStr = new StringBuilder();
                while (j < n && !END_MARKER.contains(str.charAt(j))) {
                    j++;
                }
                if (j > i + 1) {
                    modifierStr.append(str, i + 1, j);
                }

                // 转换数字和修饰符
                String target = letterMap.get(c);
     ...
     * 处理音符修饰符
     */
    private static void handleModifier(String target, String modifierStr, StringBuilder sb, ConverterContext context) {
        String[] split = target.split("[A-Gz]");
        if (split.length > 0) {
            target = target.substring(0, 1);
            modifierStr += split[1];
        ...
        while (idx < modifierStr.length()) {
            ...
            if (modifier == '#') {
                ...
                suffix_last.append(modifier);  // 连音/延音
                ...
                if (modifierStr.charAt(idx + 1) == 'h') {
                    throw new RuntimeException("暂不支持后倚音转换");
                }
                int endIdx = modifierStr.indexOf(']', idx + 1);
                String appoggiatura = modifierStr.substring(idx + 1, endIdx);
                prefix_2 = "{" + handleAppoggiatura(appoggiatura, context.letterMap).replaceAll(" ", "") + "}";
                idx = endIdx;
            }
            idx++;
        }
        ...
        context.sumTime = context.sumTime.add(Fraction.getFraction(numerator, denominator).multiplyBy(context.unit));

        if (context.sumTime.compareTo(context.mergeUnit) > 0) {
            sb.append(context.tempSection);
            context.sumTime = Fraction.getFraction(0);
            context.tempSection = "";
        } else if (context.sumTime.compareTo(context.mergeUnit) == 0) {
            sb.append(context.tempSection.replaceAll(" ", "")).append(" ");
            context.sumTime = Fraction.getFraction(0);
            context.tempSection = "";
        }
    }
    ...
    private static class ConverterContext {
        Map<Character, String> letterMap = new HashMap<>();
        Fraction mergeUnit = Fraction.getFraction(1, 4);
        Fraction unit = Fraction.getFraction(1, 4);  // L
        String tempSection = "";
        Fraction sumTime = Fraction.getFraction(0);
    }
}
