...
import { convertToDdp } from '@/utils/converter/toDdp.js'
...
async function convert() {
  ...
  const data = convertToDdp(inputText.value, mode.value, tune.value)
...
function playTone(freq, duration, volume = 0.2) {
  ...
  osc1.onended = () => {
    ...
    filter.disconnect()
    osc1.onended = null
    console.log(`音符[${playState.events[playState.index]}]音频结束，是否被提前停止：${stopped}`)
